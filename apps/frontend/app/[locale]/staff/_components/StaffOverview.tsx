...
function OfficeFloor({ office }: { office?: string | null }) {
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className="text-xl font-bold uppercase tracking-tight text-warning">
        {office}
      </span>
    </div>
  );
...
/** The brand-green "book visitors" panel: title + invite action. */
 ...
 *   row 1 — eyebrow + welcome heading (spans the stat columns) | office floor
 *   row 2 — three stat cells                                   | book panel
  ...
  return (
    <>
      ...
      <div className="hidden overflow-hidden rounded-t-xl border border-border shadow-xs lg:grid lg:grid-cols-[repeat(3,1fr)_minmax(300px,1.5fr)]">
        {/* Row 1 */}
        <div className="col-span-3 flex items-center bg-surface p-6">
          <h1 className="text-4xl font-semibold text-fg">{welcome}</h1>
        </div>
        <div className="flex flex-col justify-center border-s border-border bg-surface-muted px-6 py-5">
