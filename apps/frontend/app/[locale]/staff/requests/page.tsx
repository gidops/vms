...
/** Same status → badge mapping the VMC requests feed uses. */
...
function StaffRequests() {
  ...
  // Host-scoped: only this staff member's requests + their visitors' alerts.
  const { data, isLoading } = useInbox(tab, { scope: "mine" });

  const items = data?.items ?? [];

  return (
    <TopNavShell
      nav={<AppTopNav app="staff" active="requests" requestsCount={data?.total} />}
      filterBar={
